# QA Portfolio — Alla Kostomarova
*(English version below / Английская версия ниже)*

## 🇷🇺 Описание
Портфолио моих QA-артефактов по фронтенд- и бэкенд-тестированию мобильного фитнес-приложения **FitnessKit**.  
Документы представлены в формате PDF и включают реальные чек-листы, баг-репорты, тестовые матрицы, артефакты API- и UI-тестирования, а также элементы автоматизации.  
Все данные обезличены и безопасны для публикации.

---

## 🧩 Структура портфолио

### ⚙️ Backend Testing
- **TrainerDisableFlag — API logic and automation**  
  → Логика блокировки запросов тренерского приложения, 350+ комбинаций состояний, автоматизация Postman.
- **RulesAcceptance — API checks**  
  → Проверка обязательного ознакомления с правилами клуба.
- **AI Report — data validation**  
  → Проверка корректности расчётов и JSON-структур отчёта клиента.

### 🎨 Frontend Testing
- **RulesAcceptance — UI behaviour**  
  → Отображение модального окна, UX-устойчивость, блокировка интерфейса.
- **PersonalDataConsent — authorization logic**  
  → Новое поле «Согласие на обработку ПД» в форме авторизации.
- **Communities — testing types**  
  → Применение функционального, интеграционного и системного тестирования.
- **Communities — test design techniques & E2E**  
  → Эквивалентные классы, таблицы решений, сценарии end-to-end.

---

## 🧠 Инструменты и навыки
Postman • JSON Schema • Charles Proxy • JMeter • Selenium • Test Design •  
Regression Testing • API & UI Testing • iOS / Android QA • Documentation & Reporting

---

## 🇬🇧 English version

### Description
This portfolio showcases my QA artifacts for **frontend and backend testing** of the mobile fitness application *FitnessKit*.  
Each document (PDF) contains real examples of checklists, bug reports, test matrices, API & UI test cases, and automation elements.  
All data is anonymized and safe for public sharing.

---

### Structure

#### ⚙️ Backend Testing
- **TrainerDisableFlag — API logic and automation**  
  → Postman automation for 350+ endpoint combinations under different flag and token states.  
- **RulesAcceptance — API checks**  
  → Verification of club rules acceptance logic and configuration flags.  
- **AI Report — data validation**  
  → Validation of calculated values and JSON structures for AI reports.

#### 🎨 Frontend Testing
- **RulesAcceptance — UI behaviour**  
  → Testing modal behaviour, UX persistence, and screen blocking.  
- **PersonalDataConsent — authorization logic**  
  → New checkbox for personal data consent during login flow.  
- **Communities — testing types**  
  → Functional, integration, and system testing examples.  
- **Communities — test design techniques & E2E**  
  → Equivalence classes, decision tables, and end-to-end test scenarios.

---

### Tools & Skills
Postman • JSON Schema • Charles Proxy • JMeter • Selenium • Test Design •  
Regression Testing • API & UI Testing • iOS / Android QA • Documentation & Reporting

---

**Author:** Alla Kostomarova  
**Role:** QA Engineer (Frontend & Backend Testing)  
**LinkedIn:** [linkedin.com/in/alla-kostomarova-a9ba86216](https://linkedin.com/in/alla-kostomarova-a9ba86216)  
**GitHub:** [github.com/AllaKostomarova](https://github.com/AllaKostomarova)
